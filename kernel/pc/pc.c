                ...
                unsigned int argc, void *args, pid_t *retpid, int is_user)
{
    ...
    if (pid_alloc(&newpid))
    { //分配pid号
        kernel_printf("allocated failed!\n");
        pid_free(newpid);
        return 1;
    }
    ...
    new->task.state = PC_READY;

    kernel_printf("task create successed");
    return 0;
}
...
void clear_exited()
{
    ...
    void waitpid(pid_t pid)
    {
        ...
        if (next->parent != current_task->pid)
