...
typedef struct bootmm_info* bootInfo;
...
void set_mminfo(bootInfo info, unsigned int start, unsigned int end, unsigned int type) {
    info->start = start;
    info->end = end;
    info->type = type;
}
...
unsigned int split_mminfo(struct bootmm *mm, unsigned int index, unsigned int spStart) {
    ...
    mm->info[index].end = spStart - 1;
    mm->info[index + 1].start = spStart;
    mm->cnt_infos++;
    return 1;
}
