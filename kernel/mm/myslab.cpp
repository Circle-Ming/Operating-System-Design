...
struct kmem_cache kmalloc[PAGE_SHIFT]; 
typedef struct slab_head* sHead;
...
void init_slab() {
    ...
    for (i = 0; i < PAGE_SHIFT; i++) {
        init_each_slab(&(kmalloc[i]), size_kmem_cache[i]);
    }
#ifdef SLAB_DEBUG
    kernel_printf("Setup Slub ok :\n");
    kernel_printf("\tcurrent slab cache size list:\n\t");
    for (i = 0; i < PAGE_SHIFT; i++) {
        kernel_printf("%x %x ", kmalloc[i].objsize, (unsigned int)(&(kmalloc[i])));
    }
    kernel_printf("\n");
#endif  
}
...
void format_slabpage(struct kmem_cache *cache, struct page *page) {
    unsigned char *kmaoff = (unsigned char *)KMEM_ADDR(page, pages);  // physical addr
    struct slab_head *s_head = (struct slab_head *)kmaoff;
    void *ptr = kmaoff + sizeof(struct slab_head);
...
void *phy_kmalloc(unsigned int size) {
    ...
    // solve this
    if (size > kmalloc[PAGE_SHIFT - 1].objsize) {
    ...
    return slab_alloc(&(kmalloc[bf_index]));
}
...
void slab_free(struct kmem_cache *cache, void *object) {
    ...
    unsigned int *ptr;
    sHead s_head = (struct slab_head *)KMEM_ADDR(opage, pages);
...
unsigned int get_slab(unsigned int size) {
    ...
    for (i = 0; i < itop; i++) {
        if ((kmalloc[i].objsize >= size) && (kmalloc[i].objsize < bf_num)) {
            bf_num = kmalloc[i].objsize;
            bf_index = i;
        }
    }
    return bf_index;
